...
check_result <- function(x) {
  if (!is.logical(x))
    stop("assert_that: assertion must return a logical value")
  if (any(is.na(x)))
    stop("assert_that: missing values present in assertion")
  if (length(x) != 1) {
    stop("assert_that: length of assertion is not 1")
  }
